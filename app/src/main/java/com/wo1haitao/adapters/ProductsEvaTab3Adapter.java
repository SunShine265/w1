...
public class ProductsEvaTab3Adapter extends BaseAdapter<RsMyReview> {
    Boolean sortView;
    Context context;

    public ProductsEvaTab3Adapter(Context context, ArrayList<RsMyReview> rsMyReviews, Boolean sortView) {
        super(context, 0, rsMyReviews);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        RsMyReview rsMyReview = getItem(position);
        // Check if an existing view is being reused, otherwise inflate the view
        if (convertView == null) {
            convertView = LayoutInflater.from(getContext()).inflate(R.layout.item_eval_tab_3, parent, false);
        }
        ...
        ImageView img_product, rate1, rate2, rate3, rate4, rate5;
        ...
        try{
            imageLoader.displayImage(ApiServices.BASE_URI + rsMyReview.getProduct_listing().getProduct_images().get(0).getProduct_image().getUrl(), img_product, OPTION_DISPLAY_IMG_PRODUCT);
        ...
        try {
            String strCountry = "";
            if(rsMyReview.getProduct_listing().isNon_restricted_country() == true){
                ...
                for(int i = 0; i < rsMyReview.getProduct_listing().getCountries().size(); i++){
                    strCountry = rsMyReview.getProduct_listing().getCountries().get(i).getName();
        ...
        try {
            String reviewerName = rsMyReview.getReviewer().getNickname() + "给你的评价";
            tv_username.setVisibility(View.VISIBLE);
            tv_username.setText(reviewerName);
        }catch (Exception e){
            e.printStackTrace();
        }
